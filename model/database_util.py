...
class Batch:
    ...
    def __init__(self, attn_bias: Tensor, rel_pos: Tensor, heights: Tensor, x: Tensor, y=None):
...
def collator(small_set: List[Tuple[dict]]) -> Tuple[Batch, Tuple]:
    # Get labels
    labels = small_set[1]

    # Get features / model inputs
    features = torch.cat([s['features'] for s in small_set[0]])
    attention_bias = torch.cat([s['attention_bias'] for s in small_set[0]])
    relative_positions = torch.cat([s['rel_pos'] for s in small_set[0]])
    node_heights = torch.cat([s['node_heights'] for s in small_set[0]])
    return Batch(attention_bias, relative_positions, node_heights, features), labels


def filter_dict_to_histogram(hist_file: pd.DataFrame, filter_dict: dict, encoding) -> np.ndarray:
    """his function transforms the information in a filter dictionary into a histogram representation.
    The histogram representation includes information about the columns, operations, and values involved in the filters.
    """
    number_of_buckets = len(hist_file['bins'][0])
    empty = np.zeros(number_of_buckets - 1)
    results = np.zeros((3, number_of_buckets - 1))
    for i in range(len(filter_dict['colId'])):
        colId = filter_dict['colId'][i]
        col = encoding.idx2col[colId]
        if col == 'NA':
            results[i] = empty
            continue
        bins = hist_file.loc[hist_file['table_column'] == col, 'bins'].item()
        opId = filter_dict['opId'][0]
        op = encoding.idx2op[opId]
        val = filter_dict['val'][0]
...
class TreeNode:
    def __init__(self, node_type: str, node_type_id: int, filter: List,
                 card: int, join_id: int, join_str: str, filter_dict: dict, encoding: Encoding):

        self.nodeType = node_type
        self.operator_id = node_type_id
        self.filter = filter
        self.table = 'NA'
        self.table_id = 0
    ...
    def featurize_operator(self, hist_file: pd.DataFrame, table_sample: pd.DataFrame) -> np.ndarray:
        """operator_type_id, join_id, filter123, mask123, histograms, table_id, sample_bitmap
                          1,       1,   3x3 (9),       3,     3x50,          1,          1000
        """
        # Create filter array and mask array
        number_of_filters = len(self.filter_dict['colId'])

        # Convert values of filter dict into a numpy array
        filters = np.array(list(self.filter_dict.values()))  # cols, ops, vals

        # Create a padding array according to the number of filters found
        pad = np.zeros((3, 3 - number_of_filters))

        # Concatenate the padding array to the filter array and flatten into 1D array (flat vector)
        filters = np.concatenate((filters, pad), axis=1).flatten()

        # Create a mask for the filters according to the number of filters found
        filter_masks = np.zeros(3)
        filter_masks[:number_of_filters] = 1

        histograms = filter_dict_to_histogram(hist_file, self.filter_dict, self.encoding)

        # Add base table samples if applicable
        if self.table_id == 0:
            sample_bitmap = np.zeros(1000)
        else:
            sample_bitmap = table_sample[self.query_id][self.table]

        return np.concatenate((np.array([self.operator_id]),
                               np.array([self.join_id]),
                               filters,
                               filter_masks,
                               histograms,
                               np.array([self.table_id]),
                               sample_bitmap))
